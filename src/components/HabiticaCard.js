...
export default function HabiticaCard() {
  ...
  const updateHabiticaState = (userData) => {
    ...
    setHabiticaData({
      level: stats.lvl,
      health: { current: parseFloat(stats.hp.toFixed(0)), max: stats.maxHealth },
      experience: { current: parseFloat(stats.exp.toFixed(0)), max: stats.toNextLevel },
      mana: { current: parseFloat(stats.mp.toFixed(0)), max: stats.maxMP },
  ...
  const fetchHabiticaData = async () => {
    ...
    try {
      ...
      const response = await fetch('https://habitica.com/api/v3/user', { method: 'GET', headers });
      ...
      setHabiticaData((prev) => ({ ...prev, loading: false, error: 'Failed to load' }));
    }
  };
    ...
    <div className={styles.card}>
      ...
      <div className={styles.stats}>
        <div className={styles.statRow}>
          <span className={styles.statIcon}>HP</span>
          <div className={styles.barContainer}>
            <div className={`${styles.barFill} ${styles.hp}`} style={{ width: `${healthPercent}%` }} />
          </div>
          <span className={styles.statValue}>{habiticaData.health.current}</span>
        </div>
        <div className={styles.statRow}>
          <span className={styles.statIcon}>XP</span>
          <div className={styles.barContainer}>
            <div className={`${styles.barFill} ${styles.xp}`} style={{ width: `${expPercent}%` }} />
          </div>
          <span className={styles.statValue}>{habiticaData.experience.current}</span>
        </div>
        <div className={styles.statRow}>
          <span className={styles.statIcon}>MP</span>
          <div className={styles.barContainer}>
            <div className={`${styles.barFill} ${styles.mp}`} style={{ width: `${manaPercent}%` }} />
          </div>
